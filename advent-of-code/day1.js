...
//i will need to iterate over each character in every string, separating the numbers from the letters.  
//i will then need to find the first and last number from each string
// I will save each of these numbers as a two digit value
//then I will add all the numbers together to arrive at the answer
//i can use regex to remove letters
//map over each string and call the two functions on them
//reduce all the numbers from the new array using reduce function

const string = "9nine2xnhvjtjlzj48";

const removedLetters = (string) => {
    const reg = /[^0-9]/g;
    const resultNums = string.replace(reg, "");
    return resultNums;
};

const result = removedLetters("9nine2xnhvjtjlzj48");
// console.log(result);

const firstLast = (string) => {
    let firstNum = string.substring(0, 1);
    let secNum = string.substring(string.length - 1);
    return firstNum + secNum;
};

const finalResult = firstLast(result);
// console.log(finalResult);

const arrayofStrings = ["six1mpffbnbnnlxthree",
"4eight3one92",
"9nine2xnhvjtjlzj48",
...
const createNumArray = () =>
    arrayofStrings.map((x) => {
        const withoutLetters = removedLetters(x);
        const withFirstLast = firstLast(withoutLetters);
        
        // const sum = withFirstLast.reduce((accumulator, currentValue) => {
        //     return accumulator + currentValue
        // }, 0)
        const numberValue = parseInt(withFirstLast, 10)
        return numberValue
    ...
    const sum = createNumArray().reduce((accumulator, currentValue) => {
        return accumulator + currentValue
    }, 0)
